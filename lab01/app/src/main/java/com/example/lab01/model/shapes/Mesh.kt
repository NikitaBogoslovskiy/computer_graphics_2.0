...
class Mesh(
    ...
    private val bumpData: TextureData
    private var bumpStep = FloatArray(2)
    ...
    init {
        textureData1 = Dependencies.textureLoader.loadTexture(R.drawable.default_texture)
        textureData2 = Dependencies.textureLoader.loadTexture(R.drawable.default_texture)
        bumpData = Dependencies.textureLoader.loadTexture(bumpTextureId)
        bumpStep[0] = 1f / bumpData.width
        bumpStep[1] = 1f / bumpData.height
        processData()
        ...
        phongProgram = GLES20.glCreateProgram().also {
            GLES20.glAttachShader(it, loadShader(GLES20.GL_VERTEX_SHADER, PHONG_VERTEX_SHADER_WITH_BUMP_MAPPING))
            GLES20.glAttachShader(it, loadShader(GLES20.GL_FRAGMENT_SHADER, PHONG_FRAGMENT_SHADER_WITH_BUMP_MAPPING))
            GLES20.glLinkProgram(it)
        }
    ...
    private fun setTexturesParams(program: Int) {
        ...
        val textureBumpLoc = GLES20.glGetUniformLocation(program, "texture_bump")
        val bumpStepLoc = GLES20.glGetUniformLocation(program, "bump_step")
        ...
        GLES20.glUniform1i(textureBumpLoc, bumpData.textureNumber)
        GLES20.glUniform2fv(bumpStepLoc, 1, bumpStep, 0)
        ...
        GLES20.glActiveTexture(GLES20.GL_TEXTURE0 + bumpData.textureNumber)
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, bumpData.textureId)
    }
